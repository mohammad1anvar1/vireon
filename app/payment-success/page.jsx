  ...
  if (state === 'error') return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.muted}>Please contact support@vireonai.uk</p>
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.subtitle}>Sent to {order?.customerEmail}</p>
        <div style={styles.amountBox}>
          <span style={styles.amount}>£{((order?.amountTotal||0)/100).toFixed(2)}</span>
          <span style={styles.amountLabel}> paid</span>
        </div>
        <div style={styles.details}>
          {[['Order',order?.orderNumber],['Product',order?.productName],['Email',order?.customerEmail],['Date',new Date().toLocaleDateString('en-GB')]].map(([k,v])=>(
        ...
        <div style={styles.btnRow}>
          <a href={`/api/payment/download-report?session=${sessionId}`} style={styles.btnPrimary}>Download Report</a>
          <button style={styles.btnSecondary} onClick={()=>router.push('/dashboard')}>Dashboard</button>
        </div>
      </div>
    </div>
  );
...
export default function PaymentSuccessPage() {
  return (
    <Suspense fallback={<div style={{minHeight:'100vh',background:'#06060a',display:'flex',alignItems:'center',justifyContent:'center',color:'#e8ff3c',fontFamily:'Georgia,serif'}}>Loading...</div>}>
      <SuccessContent />
    </Suspense>
  );
}
...
const styles = {
  ...
  spinnerWrap:{display:'flex',justifyContent:'center',marginBottom:'16px'},
  spinner:{width:'36px',height:'36px',border:'2px solid rgba(255,255,255,0.1)',borderTopColor:'#e8ff3c',borderRadius:'50%'},
